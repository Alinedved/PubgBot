...
class pubg():
	...
	async def getID(self,playername):
		...
		return {'playerid':playerid,'nickname':nickname}
		...
		if len(players)==0:
			...
			for player in players:
				...
				if 'error' not in data:
					...
					url='https://pubg.op.gg/api/users/{0.playerid}/ranked-stats?season=2018-08&server={0.server}&queue_size={0.size}&mode={0.match}'.format(profile)
					player_stats = await self.PUBG_API(url)
					print (profile.playerid)
					...
					if 'message' in player_stats:
						...
						embed.add_field(name="Longest Kill", value=player_stats['stats']['longest_kill_max'], inline=True)
						embed.add_field(name='Max Kills', value=player_stats['stats']['kills_max'], inline=True)
						embed.add_field(name='Discord', value='https://discord.gg/8J2RKB9', inline=True)
						await self.bot.say(embed=embed)
				else:
